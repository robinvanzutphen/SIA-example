...
for c = 1:nC
    ...
    s1 = scatter(ax, Ps(1:Ns,1), Ps(1:Ns,2), 6, 'filled', 'DisplayName','Input (source)');
    s2 = scatter(ax, Pd(1:Nd,1), Pd(1:Nd,2), 6, 'filled', 'DisplayName','Output (detector)');
    ...
    if c == 1
        legend(ax, [s1 s2], {'Input (source)','Output (detector)'}, 'Location','best');
    end
end
end
...
switch C.type
    case 'uniform_shapes'
        ...
        switch C.det_shape
            case {'disk','annulus'}
                rd = norm(C.det_offset_mm) + C.det_rout;
            otherwise
                rd = norm(C.det_offset_mm) + 0.5*C.det_side;
        end
    ...
    case 'gaussian_overlap'
        lim = 1.10 * (C.R_det_mm + C.Nw_cap*C.w_mm);
